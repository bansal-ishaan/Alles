...
export default function ChatRoomPage() {
    ...
    const initializeRoom = async () => {
      ...
      try {
        const joinRes = await fetch(
          `${serverUrl}/api/v1/rooms/join/${roomId}`,
          {
            method: "POST",
        ...
        if (!joinRes.ok)
          throw new Error("Room not found or you could not be added.");
        ...
        const uniqueMembers = Array.from(
          new Map(
            roomData.data.members.map((m: Member) => [m._id, m]),
          ).values(),
        );
  ...
  if (isLoading)
    return (
      <div className="flex items-center justify-center h-screen bg-discord-gray-700 text-white">
        Loading Room...
      </div>
    );
  if (error)
    return (
      <div className="flex items-center justify-center h-screen bg-discord-gray-700 text-red-400">
        Error: {error}
      </div>
    );
  ...
  const onlineMembers = roomDetails.members.filter((m) =>
    onlineUserIds.has(m._id),
  );
  const offlineMembers = roomDetails.members.filter(
    (m) => !onlineUserIds.has(m._id),
  );
    ...
    <div className="flex h-full text-gray-300 font-sans">
      ...
      <div className="flex-1 flex flex-col  border-r-white border-r-1">
        <header className="flex-shrink-0 flex items-center p-3 border-b-2  shadow-md z-10">
          ...
          <h1 className="ml-2 text-md align-right text-discord-gray-500">
            ({roomId})
          </h1>
        </header>
            ...
            {Object.entries(groupedMessages).map(([date, messagesOnDate]) => (
              <div key={date}>
                ...
                {messagesOnDate.map((msg) => (
                  <div
                    key={msg._id}
                    className="group relative flex items-start gap-4 p-2 rounded hover:bg-gray-900/40 transition-colors"
                  >
                    <img
                      src={
                    ...
                    <div>
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-xs text-discord-gray-500">
        ...
        <footer className="px-4 pb-6">
          ...
          >
            <button
              type="button"
              className="absolute left-4 top-1/2 -translate-y-1/2 text-discord-gray-500 hover:text-white"
            >
              <PlusCircle size={24} />
            </button>
            <input
      ...
      <aside className="w-60 bg-discord-gray-800  flex flex-col">
        <div className="flex-1 overflow-y-auto p-3 space-y-4 scrollbar-thin scrollbar-thumb-discord-gray-900 scrollbar-track-discord-gray-800">
          {/* Online Members */}
          <div className="text-xs font-bold text-discord-gray-500 uppercase">
            Online — {onlineMembers.length}
          </div>
          {onlineMembers.map((member) => (
            <div
              key={member._id}
              className="group flex items-center justify-between p-1 rounded"
            >
              <div className="flex items-center gap-3">
                <img
                  src={member.avatar?.url || "https://via.placeholder.com/32"}
                  alt={member.username}
              ...
              {isOwner && currentUser._id !== member._id && (
                <button
                  ...
                  className="text-discord-gray-500 hover:text-red-500 opacity-0 group-hover:opacity-100"
                >
                  <UserX size={16} />
                </button>
              )}
          ...
          {/* Offline Members */}
          <div className="text-xs font-bold text-discord-gray-500 uppercase pt-4">
            Offline — {offlineMembers.length}
          </div>
          {offlineMembers.map((member) => (
            <div
              key={member._id}
              className="group flex items-center justify-between p-1 rounded opacity-40"
            >
              <div className="flex items-center gap-3">
                <img
                  src={member.avatar?.url || "https://via.placeholder.com/32"}
        ...
        {isOwner && (
          <button
            ...
            className="w-full text-left p-4 mt-auto border-t-2 border-discord-gray-900 text-red-500 hover:bg-red-500/10 flex items-center gap-2"
          >
