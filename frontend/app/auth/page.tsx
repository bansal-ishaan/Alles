...
const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        if (err instanceof Error) {
            setError(err.message); // ✅ show actual server error
        } else {
    ...
    <div className="max-w-md mx-auto mt-12 p-8 border border-gray-700 rounded-lg bg-gray-900 text-white">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <>
                    <div>
                        ...
                        <input
                            ...
                            className="w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:font-semibold file:bg-gray-700 text-gray-300 hover:file:bg-gray-600"
                            required
                        />
                    </div>
                    <div>
            ...
            <button
                type="submit"
                disabled={isLoading}
                className="w-full p-2 text-white bg-blue-600 rounded hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed"
            >
        ...
        {error && (
            <p className="mt-4 text-center text-sm text-red-400 bg-red-900/50 p-3 rounded">
                {error}
            </p>
        )}
        {successMessage && (
            <p className="mt-4 text-center text-sm text-green-400 bg-green-900/50 p-3 rounded">
                {successMessage}
            </p>
        )}
